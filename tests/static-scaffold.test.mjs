...
test("static scaffold files include required shell hooks", async () => {
  ...
  assert.match(readme, /一个静态的 Markdown 简历编辑器/);
  assert.match(readme, /https:\/\/nozomix1\.github\.io\/oh-my-resume\//);
  assert.match(readme, /npm run serve/);
  assert.match(readme, /打开 <http:\/\/localhost:4173>。/);
  assert.match(readme, /npm test/);
  assert.match(readme, /点击 `Print \/ Save PDF`，然后在系统打印窗口中选择 `Save as PDF`。/);
  assert.match(readme, /推荐使用 Chrome 导出 PDF/);
  assert.match(readme, /Background graphics/);
  assert.match(readme, /这个项目没有构建步骤/);
